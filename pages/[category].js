...
const CategoryPage = ({
  ...
  const [productsOfCategoriesLastPage, setProductsOfCategoriesLastPage] =
    useState(productsOfCategory.data?.lastPageNo);
  const [selectPage, setSelectPage] = useState(currentPage);
  const [selectedSubCategory, setSelectedSubCategory] = useState("");
  const [advancedsubCategory, setAdvancedSubCategory] = useState([]);
  const [advancedColor, setAdvancedColor] = useState([]);
  ...
  const endProduct = Math.min(
    ...
    productsOfCategory.data?.totalProductCount
  );
  ...
  useEffect(() => {
    axios
      .get(
        `${Environment.baseURL
        }/api/AdvancedSearch/getBasicData?categoryId=${category}&subCategoryId=${selectedSubCategory ? selectedSubCategory : ""
        }`,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchFilteredProducts = () => {
    ...
    axios
      .post(
        ...
        {
          categoryId: category,
          subCategoryIds: selectedFilterSubCategory
  ...
  return (
    <>
      <section
        className={`${styles.category} ${code == "AR" ? zain.className : PlaywriteDEGrund.className
          }`}
        dir={dir}
      >
        <div className={`${styles.category__top}`}>
          <h2>{translations.theProducts}</h2>
          <p>{prodName}</p>
        </div>
        <div>
          <div className={`${styles.filter__responsive}`} onClick={handleShowResponsive}>
            <FilterAltIcon />
            <span>{translations.FilterProducts}</span>
            <hr />
          </div>
          <Modal show={showResponsive} onHide={handleCloseResponsive} dir={dir}>
            <Modal.Header closeButton>
              <Modal.Title>{translations.FilterProducts}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <Accordion>
                <Accordion.Item eventKey="0">
                  <Accordion.Header className={`header__acc`}>{translations.subCategory}</Accordion.Header>
                  <Accordion.Body>
                    <div className={`${styles.sub__responsive}`}>
                      {advancedsubCategory?.map((subCat) => (
                        <div className="checkbox-wrapper" key={subCat.id}>
                          <input
                            type="checkbox"
                ...
                <Accordion.Item eventKey="1">
                  <Accordion.Header className={`header__acc`}>{translations.colorr}</Accordion.Header>
                  <Accordion.Body>
                    <div className={`${styles.sub__responsive}`}>
                      {advancedColor?.map((colorr) => (
                        <div className="checkbox-wrapper" key={colorr.id}>
                ...
                <Accordion.Item eventKey="2">
                  <Accordion.Header className={`header__acc`}>{translations.sizz}</Accordion.Header>
                  <Accordion.Body>
                    <div className={`${styles.sub__responsive}`}>
                      {advancedSize?.map((size) => (
                        <div className="checkbox-wrapper" key={size.id}>
                ...
                <Accordion.Item eventKey="3">
                  <Accordion.Header className={`header__acc`}>{translations.price}</Accordion.Header>
                  <Accordion.Body>
                    <Box>
                      <Slider
                        value={price}
                ...
                <Accordion.Item eventKey="4">
                  <Accordion.Header className={`header__acc`}>{translations.Sortbyprice}</Accordion.Header>
                  <Accordion.Body>
                    <div className="checkbox-wrapper">
                      <input
                        type="radio"
          ...
          <div className={`${styles.category__center}`}>
            ...
            <div className={`${styles.view__body}`}>
              <h6>
                {translations.Showing} {startProduct} - {endProduct}{" "}
                {translations.of} {productsOfCategory.data?.totalProductCount}{" "}
                {translations.products}
              </h6>
              <div className={`${styles.gridOrList}`}>
                <div onClick={() => setGridOrList("grid")}>
                  <ViewCompactIcon />
                </div>
                <div onClick={() => setGridOrList("list")}>
          ...
          {isDropdownOpen && (
            <div className={`${styles.popDown}`} dir={dir}>
              <div
                className={`${styles.dropdowncontent} ${code == "AR" ? zain.className : PlaywriteDEGrund.className
                  }`}
              >
                <Row className={`${styles.filter__row}`}>
                  <Col>
                    <h5>{translations.subCategory}</h5>
                    {advancedsubCategory?.map((subCat) => (
                      <div className="checkbox-wrapper" key={subCat.id}>
                        <input
                          type="checkbox"
                  ...
                  <Col>
                    <h5> {translations.Sortbyprice}</h5>
                    <div className="checkbox-wrapper">
                      <input
                        type="radio"
                        checked={selectedOption == true}
